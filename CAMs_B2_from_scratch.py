...
#resnet50.load_state_dict(resnet50_radimagenet_weights)
#------------------------------------------------------------------------------
#mod = nn.Sequential(*list(resnet50.children())[:-2])# remove fully connected layer and GlobalAvgPool
mod = nn.Sequential(*list(resnet50.children())[:-3])# for ImageNet. Remove fully connected layer and GlobalAvgPool and last conv block

# this net is from https://debuggercafe.com/pytorch-class-activation-map-using-custom-trained-model/
...
torch.save(model.state_dict(), 'B2scratch_100epochs_ImageNet.pt')
"=============================================================================="
...
state_dict = torch.load('/home/daa/Desktop/Grace_ML_Labs/Code/B2scratch_100epochs_ImageNet.pt', map_location=torch.device(device))
...
for fname in range(1):#IMG_URL:
    with torch.no_grad():
        fname = '27.png'#{2,7,8,10,17,25,27,28,31,39,42} 
        ...
        features_blobs = model(img_variable)
...
#---- comment either one ------------------------------------------------------    
        ...
        EigenCAMs = EigenCAM(model, [model.conv2])
